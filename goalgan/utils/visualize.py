...
def visualize_probabilities(y_softmax=None,
    ...
    recon_img = np.clip(y[0].cpu().numpy(), 0, 1)
    ...
    softmax_img = torch.clip(y_softmax[0].cpu().unsqueeze(0), 0, 1)
    # softmax_img /= np.max(softmax_img)
    # softmax_img = Image.fromarray(softmax_img)
    # softmax_img = ToTensor()(np.array(
    #     softmax_img.resize(
    ...
    softmax_img = softmax_img / (softmax_img.max() + 1e-6)/2
...
def visualize_trajectories(input_trajectory=None,
    ...
    extent = [center[0] - dx, center[0] + dx, center[1] + dx, center[1] - dx]

    axs.scatter(input_trajectory[:, 0], input_trajectory[:, 1], color="blue", marker="o", s=2)
    marker_style = dict(color='tab:orange', marker='o',
                        markersize=3, markerfacecoloralt='tab:red', markeredgewidth=1)
    gt_trajectory = gt_trajectory[:, 0].cpu()
    gt_trajectory = torch.clip(gt_trajectory, 0, 224)
    axs.plot(gt_trajectory[:, 0].cpu(), gt_trajectory[:, 1].cpu(), color="orange")
...
def visualize_traj_probabilities(input_trajectory=None,
    ...
    visualize_probabilities(y_softmax=y_softmax[0],
                            y=y[0],
                            global_patch=global_patch[0],
