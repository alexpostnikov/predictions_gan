...
from data.waymoDSGoalGan import create_rot_matrix
...
class AttentionRoutingModule(AttentionNetwork):
    ...
    def forward(self, scene_img, last_pos, h, noise=torch.Tensor()):
        ...
        img_patch = self.img_patch.gen_patch_waymo(scene_img, last_pos)
...
class GoalGlobal(VisualNetwork):
    ...
    def forward(self, features, h, pool_h=torch.empty(1)):
        ...
        # final_pos : 1, bs ,2; final_pos_map_decoder: bs,1, 33 ,33, final_pos_map_decoder: bs,1, 33 ,33, y_softmax: bs,1, 33 ,33; y_scores: bs, 33*33
...
class RoutingModule(nn.Module):
    ...
    def forward(self, last_pos, rel_pos, state_tuple, dist_to_goal=0, scene_img=None):
        ...
        for t in range(self.seq_len):
            ...
            # TODO
            if self.rm_vis_type == "attention":
                final_emb, y_softmax, img_patch, noise = self.rm_attention(scene_img, rel_pos, state_tuple[0])
            else:
        ...
        # create_rot_matrix(pred_traj_fake, bbox, past_valid)
...
class Patch_gen():
    ...
    @staticmethod
    def gen_patch_waymo(img, pos, size=(33, 33)):
        """
        Generate a patch of size size from the image at position pos
        :param img: bs, 3, 224, 224
        :param pos: bs, 2
        :param size:
        :return:
        """
        ...
        # image pixels in local CS, not global,
        # pose 0m,0m corresponds to pixels position 224/4, 224/2
        shift = torch.tensor([[224/4, 224/2]]).to(pos)
        pos_pix = pos * m_to_pix + shift
        pos_pix = torch.clamp(pos_pix, 16, 224-17)
        # create pashes around pos_pixel
        out_pathes = torch.zeros(img.shape[0], 3, size[0], size[1]).to(pos)
        for i in range(img.shape[0]):
            out_pathes[i] = img[i][:, int(pos_pix[i][1]) - size[1]//2: int(pos_pix[i][1]) + size[1]//2+1,
                                   int(pos_pix[i][0]) - size[0]//2: int(pos_pix[i][0]) + size[0]//2+1]   # bs, 3, 33, 33
        return out_pathes
    ...
    def get_patch(self, scene_image, last_pos):
        ...
        for k in range(len(scene_image)):
            ...
            cropped_img=image[:, max(0,int(x_center - self.grid_size)):int(x_center + self.grid_size + 1),
                                 max(0,int(y_center - self.grid_size)):int(y_center + self.grid_size + 1)]
            cropped_img = torch.cat([cropped_img, torch.zeros(cropped_img.shape[0],
                                                             cropped_img.shape[1],
                                                             self.grid_size * 2 + 1 - cropped_img.shape[2])], dim=2)
            cropped_img = torch.cat([cropped_img, torch.zeros(cropped_img.shape[0],
                                                              self.grid_size * 2 + 1 - cropped_img.shape[1],
                                                              cropped_img.shape[2]).to(cropped_img.device)], dim=1)
            position = torch.zeros((1, self.grid_size * 2 + 1, self.grid_size * 2 + 1, 1))
            position[0, self.grid_size, self.grid_size, 0] = 1.
            image = torch.cat((cropped_img.float().unsqueeze(0).permute(0,2,3,1), position), dim=3)
...
if __name__ == "__main__":
    ...
    print(MotionEncoder())
    me = MotionEncoder()
    # create Tensor of shape (obs_len, batch, 2)
    obs_len = 8
    batch = 2
    obs = torch.rand(obs_len, batch, 2)
    encoder_out, h_enc = me(obs)

    print("Test GoalGlobal")
    print(GoalGlobal())
    gg = GoalGlobal()
    # create random image of shape (batch, 4, 224, 224)
    img = torch.rand(batch, 4, 32, 32)
    gg_out = gg(img, h_enc.reshape(-1, 128))

    print("Test Decoder")
    last_pos = torch.rand(2, 2)
    rel_pos = torch.rand(2, 2)
    state_tuple = (torch.rand(1, 2, 128), torch.rand(1, 2, 128))
    print(RoutingModule())
    rm = RoutingModule()
    scene_img = torch.rand(2, 3, 64, 64)
    rm_out = rm(last_pos, rel_pos, state_tuple, dist_to_goal=torch.rand(1, 2, 2), scene_img=scene_img)
