...
class GoalGAN(BaseModel):
    ...
    def forward(self, batch, final_pos_in=torch.Tensor()):
        ...
        final_pos, final_pos_map_concrete, final_pos_map, y_softmax, y_scores = self.goalmodule(batch["global_patch"],
                                                                                                h_enc)
        out = {
        ...
        v0 = batch["in_dxdy"][-2]
        state_tuple = (h, c)
        rm_out = self.routingmodule(last_pos=x0, dist_to_goal=final_pos, rel_pos=v0, state_tuple=state_tuple,
