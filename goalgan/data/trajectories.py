...
class TrajectoryDataset(Dataset):
    ...
    def gen_global_patches(self, scene_image, trajectory, prediction, image_type="global_image"):
        """
        Generate global patches from scene image and trajectory
        :param scene_image:
        :param trajectory:
        :param prediction:
        :param image_type:
        :return: global_patch: (4, patch_size, patch_size), prob_mask: (1, patch_size, patch_size)
        # patch size = self.grid_size_in_global * 2 + 1
    ...
    def data_aug_func(self, scene=None,
        ...
        return obs_traj, pred_traj, dxdy[:, :self.obs_len - 1], dxdy[:,
                                                                self.obs_len - 1:], scene_image, global_patch, prob_mask_tensor, walls, local_patch
