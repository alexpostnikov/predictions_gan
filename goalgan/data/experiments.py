...
class Experiment:
    ...
    def warp_obstacle(self):
        ...
        self.world_obst = cv2.warpPerspective(im_src, h, (
        int(np.max(corners_real[:, 0]) / self.scaling), int(np.max(corners_real[:, 1]) / self.scaling)),
                                              borderValue=(255, 255, 255))
        self.world_stat = cv2.warpPerspective(stat_img, h, (
        int(np.max(corners_real[:, 0]) / self.scaling), int(np.max(corners_real[:, 1]) / self.scaling)),
                                              borderValue=(255, 255, 255))
    ...
    def save_shift(self):
        ...
        np.savetxt(os.path.join(self.data_path, "{}.txt".format(self.name)), self.world_shifted[:, :4],
                   fmt="%i\t%i\t%1.2f\t%1.2f")
