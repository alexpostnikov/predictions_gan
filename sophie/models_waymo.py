...
class PhysicalAttention(nn.Module):
    def __init__(self):
        ...
        self.spatial_embedding = nn.Sequential(nn.Linear(2, self.embedding_dim), nn.BatchNorm1d(self.embedding_dim))
        self.pre_att_proj = nn.Sequential(nn.Linear(self.D, self.D_down*self.L), nn.BatchNorm1d(self.D_down*self.L))
        ...
        self.attn = nn.Sequential(nn.Linear(self.L*self.bottleneck_dim, self.L), nn.BatchNorm1d(self.L))
    ...
    def forward(self, vgg, end_pos):
        ...
        features_proj = features_proj.view(features_proj.shape[0], -1, self.D_down)
        curr_rel_embedding = curr_rel_embedding.view(-1, 1, self.embedding_dim).repeat(1, features_proj.shape[1], 1)
        ...
        attn_h = attn_h.view(npeds, features_proj.shape[1], self.bottleneck_dim)
...
class SocialAttention(nn.Module):
    def __init__(self):
        ...
        self.spatial_embedding = nn.Sequential(nn.Linear(2, self.embedding_dim), nn.BatchNorm1d(self.embedding_dim))
        self.mlp_pre_attn = make_mlp(mlp_pre_attn_dims)
        self.attn = nn.Sequential(nn.Linear(MAX_PEDS*self.bottleneck_dim, MAX_PEDS), nn.BatchNorm1d(MAX_PEDS))
    ...
    def forward(self, h_states, end_pos):
        ...
        curr_rel_embedding = self.spatial_embedding(curr_rel_pos.reshape(-1, 2))
...
class TrajectoryGenerator(nn.Module):
    ...
    def forward(self, obs_traj, obs_traj_rel, img):
